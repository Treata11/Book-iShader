/*
 ...
 Abstract:
 The NoteProcessing class specifies some constants and variable specific to musical notes.
...
/**
 ...
 These 96 notes cover 8 octaves.  It is insightful to consider these 96 notes as a **two-dimensional** grid with one octave per row:
 
 >       Note       Freq              Note        Freq
 | Row | Left Note | Left Freq (Hz) | Right Note | Right Freq (Hz) |
 |------ |------------|-----------------|------------- |------------------|
 | 7      | C8           | 4186.01          | B8              | 7902.14           |
 | 6      | C7           | 2093.00          | B7              | 3951.07           |
 | 5      | C6           | 1046.50          | B6              | 1975.53           |
 | 4      | C5           | 523.25            | B5              | 987.77             |
 | 3      | C4           | 261.63            | B4              | 493.88             |
 | 2      | C3           | 130.81            | B3              | 246.94             |
 | 1      | C2           | 65.41              | B2              | 123.47             |
 | 0      | C1           | 32.70              | B1              | 61.74               |

 
 ## Note Names:
 | Row|Left Note | Left Freq | Right Note | Right Freq | bottomBin| binsInRow| topBin|
 |-----|--------------|------------|-----------|------------|-----------|-----------|---------|
 |  7   | C8              | 4186.01   | B8         | 7902.14    | 1511      | 1511      | 3021   |
 |  6   | C7              | 2093.00   | B7         | 3951.07    | 756        | 755        | 1510   |
 |  5   | C6              | 1046.50   | B6         | 1975.53    | 378        | 378        | 755     |
 |  4   | C5              | 523.25     | B5         |  987.77     | 189        | 189        | 377     |
 |  3   | C4              | 261.63     | B4         | 493.88      | 95          | 94          | 188     |
 |  2   | C3              | 130.81     | B3         | 246.94      | 48          | 47          | 94       |
 |  1   | C2              | 65.41       | B2         | 123.47      | 24          | 24          | 47       |
 |  0   | C1              | 32.70       | B1         | 61.74        | 12          | 12          | 23       |
 
 | Row | Left Freq (Hz) | Bottom Bin | Bins in Row | Top Bin | Right Freq (Hz) |
 |------ |----------------|---------------|---------------|----------|------------------ |
 | 7      | 4066.84         | 1511            | 1511            | 3021     | 8133.68            |
 | 6      | 2033.42         | 756              | 755              | 1510     | 4066.84            |
 | 5      | 1016.71         | 378              | 378              | 755       | 2033.42            |
 | 4      | 508.36           | 189              | 189              | 377       | 1016.71            |
 | 3      | 254.18           | 95                | 94                | 188       | 508.36              |
 | 2      | 127.09           | 48                | 47                | 94         | 254.18              |
 | 1      | 63.54             | 24                | 24                | 47         | 127.09              |
 | 0      | 31.77             | 12                | 12                | 23         | 63.54                |

 
 We will abide by the international standard wherein the note A4 has the exact frequency of 440 Hz.
 Since an octave is a doubling of frequency, we can state that:
 freqA1 = 55 Hz, freqA2 = 110 Hz, freqA3 = 220 Hz, freqA4 = 440 Hz,  freqA5 = 880 Hz,  freq A6 = 1760 Hz,  freqA7 = 3520 Hz, and freqA8 = 7040 Hz.
 
 For the equal-temperment scale, we calculate the frequency of adjacent notes by multiplying/dividing by the twelfth root of two.
 We can calculate the "half-way points" separating adjacent notes by multiplying/dividing by the twenty-fourth root of two.
 For the note C1, we denote it's lower frequency half-way point by "leftFreqC1", and it's upper frequency half-way point by "rightFreqC1".
 
 In the BassAudioEngine class, we specify that the audio sampleRate is 44,100 samples-per-second, and that the fftLength is 16,384 samples.
 Hence, the spectrum contains 8,192 frequency "bins" - separated by binFreqWidth = (44100 / 2) / 8192 = 2.69165 Hz
 
 This NoteProcessing class calculates various parameters relating the frequency bins provided by the BassAudioEngine's FFT function
 to the octaves and notes of interest to musicians.
 */
class NoteProcessing {
    ...
    func calculateParameters() {
        ...
        DispatchQueue.global().async { [self] in
            ...
            let binFreqWidth = (Double(BassAudioEngine.sampleRate)/2) / Double(BassAudioEngine.binCount) // (44100/2)/8192=2.69165 Hz
            ...
            for oct in 0 ..< 8 {    // 0 <= oct < 8
                for bin in octBottomBin[oct] ... octTopBin[oct] {
                    ...
                    let freqFraction: Double = pow(Double(twelfthRoot2), 12 * binFraction) // 1.0 < freqFraction < 2.0
                    
                    // This is an array of scaling factors to multiply the octaveWidth to get the x coordinate:
                    // That is, binXFactor goes from 0 to 1.0 within each octave.
                    binXFactor[bin] =  (2 - (2 / freqFraction))
                    ...
                    if(oct < 6) { binXFactor6[bin] = ( Double(oct) + binXFactor[bin] ) / 6 }
                    
                    // As bin goes from 12 to 94, binXFactor3 goes from 0 to 1.0
                    if(oct < 3) { binXFactor3[bin] = ( Double(oct) + binXFactor[bin] ) / 3 }
                }
            }
            ...
            for oct in 3 ... 5 {    // 3 <= oct <= 5
                for bin in octBottomBin[oct] ... octTopBin[oct] {
                    ...
                    binXFactor36[bin] = ( ( Double(oct) + binXFactor[bin] ) / 3 ) - 1
                    // count += 1
            ...
            for point in 0 ..< sixOctPointCount {           // sixOctPointCount = 72 * 12 = 864
                ...
                cos2PiTheta[point] = cos(2 * Double.pi * theta)
                sin2PiTheta[point] = sin(2 * Double.pi * theta)
            }
        }
    ...
    /**
     The spectrum (as calculated in the BassAudioEngine class) uses spectral values on a linear-frequency scale.
    ...
    */
    public func computeMuSpectrum(inputArray: [Float]) -> [Float] {
        ...
        let tempFloat3 = Float(BassAudioEngine.binFreqWidth)
    ...
    public func computeHarmProdSpectrum(inputArray: [Float]) -> [Float] {
        ...
        var harmProdSpectrum = [Float] (repeating: 1, count: sixOctPointCount) // sixOctPointCount = 72*12 = 864
